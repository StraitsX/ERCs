...
contract PBMERC20Adapter {
    ...
    constructor(address _pbmContract, uint256 _tokenId) {
        pbmContract = _pbmContract;
        tokenId = _tokenId;
    }

    // Implement ERC-20 functions (balanceOf, allowance, approve, etc.) as wrappers around PBM logic
}
```
...
PBMERC20Adapter adapter = new PBMERC20Adapter(address(pbm), 0); // 0 = tokenId
```
