...
interface IPBMRC1 is IERC173, IERC5679Ext1155 {
    ...
    /// @notice Processes payment request from customer to merchant, unwrapping PBM tokens upon successful validation
    /// @dev This function handles the merchant payment flow, performing necessary validations and unwrapping the underlying sovToken
    /// @param from Wallet address to deduct PBM from
    /// @param to Payment destination address (must be a whitelisted merchant address)
    /// @param id PBM Token identifier
    /// @param amount Number of PBM tokens to send
    /// @param sourceReferenceID Unique payment identifier for tracking and validation purposes
    /// @param data Additional metadata for payment processing
    function requestPayment(address from, address to, uint256 id, uint256 amount, string memory sourceReferenceID, bytes memory data) external;
...
Some developers and wallet providers may prefer to interact with PBM tokens using the widely supported ERC-20 interface, rather than ERC-1155. This extension provides an adapter that exposes PBM tokens as ERC-20 tokens, enabling seamless integration with existing wallets and infrastructure that do not natively support ERC-1155.

##### Motivation

While ERC-1155 offers flexibility and multi-token support, ERC-20 remains the most widely adopted token interface for wallets and dApps. The ERC-20 Adapter allows PBM tokens to be represented and transferred as ERC-20 tokens, improving compatibility and user experience.

##### Specification

The ERC-20 Adapter acts as a bridge between the PBM (ERC-1155) contract and the ERC-20 interface. It allows users to interact with a specific PBM tokenId as if it were a standard ERC-20 token, while internally invoking the appropriate PBM logic.
...
/// @title ERC-20 Adapter for PBM
/// @notice This contract wraps a specific PBM tokenId as an ERC-20 token for compatibility with ERC-20 wallets and dApps.
contract PBMERC20Adapter {
    address public pbmContract;
    address public eventSourceContract;
    uint256 public tokenId;

    constructor(address _pbmContract, address _eventSourceContract, uint256 _tokenId) {
        pbmContract = _pbmContract;
        eventSourceContract = _eventSourceContract;
        tokenId = _tokenId;
    }

    /// @notice Standard ERC-20 transfer function that maps to PBM safeTransferFrom for peer-to-peer transfers
    /// @dev This performs PBM peer-to-peer transfer without unwrapping
    function transfer(address to, uint256 amount) external returns (bool);

    /// @notice Custom function required for PBM payment requests to merchants
    /// @dev This function is necessary for unwrapping PBM tokens when paying merchants
    /// Wallet providers MUST implement support for this function to enable full PBM functionality
    /// @param from Wallet address to deduct PBM from
    /// @param to Payment destination address (must be a whitelisted merchant address)
    /// @param amount Number of PBM tokens to send
    /// @param sourceReferenceID Unique payment identifier for tracking and validation
    /// @param data Additional metadata for payment processing
    function requestPayment(address from, address to, uint256 amount, string memory sourceReferenceID, bytes memory data) external returns (bool);
    }

    // Implement other ERC-20 functions (balanceOf, allowance, approve, etc.) as wrappers around PBM logic

```

##### Usage

- Each PBM token type (tokenId) can have a corresponding ERC-20 Adapter contract.
- The adapter exposes standard ERC-20 functions, internally mapping them to the PBM's ERC-1155 logic.
- This enables wallets and dApps that only support ERC-20 to interact with PBM tokens transparently.

##### Limitations and Requirements
- Wallet providers still need to implement the requestPayment function to enable merchant payments and PBM unwrapping. Standard ERC-20 transfer only supports peer-to-peer transfers without unwrapping.
- Wallet providers must subscribe to events from both the adapter contract and the underlying PBM contract (and potentially separate event source contracts) to receive complete transaction information.

##### Example

```solidity
// Example usage of the adapter
PBMERC20Adapter adapter = new PBMERC20Adapter(
    address(pbmContract),      // Main PBM contract address
    address(eventSource),      // Event source contract (may be same as PBM contract)
    0                         // tokenId
);

// Standard ERC-20 transfer (peer-to-peer, no unwrapping)
adapter.transfer(userAddress, amount);
```
...
### Why provide an ERC-20 Adapter?

This adapter pattern allows for maximum compatibility and ease of integration, especially for wallets and services that do not yet support ERC-1155. It also enables a smoother user experience for end-users who expect ERC-20 style tokens.

### Why use an ERC-20 Adapter rather than a native ERC-20 PBM?

Why not just create native ERC-20 PBM contracts?

- Creating separate ERC-20 contracts for each PBM token type would increase deployment costs and complexity, as each token type (different denominations, expiry dates, business logic) would require its own contract deployment.
- Managing multiple separate ERC-20 PBM contracts increases maintenance burden compared to a single ERC-1155 contract with optional adapters.

Why is the adapter approach better?

- The adapter approach allows developers to choose their preferred interface (ERC-20 or ERC-1155) without sacrificing the underlying architectural benefits of the multi-token standard.
- Systems can interact with the same PBM tokens through both ERC-20 and ERC-1155 interfaces simultaneously, providing maximum flexibility for different use cases and integrations.
- The ERC-20 adapter approach maximizes compatibility while maintaining the benefits brought by usage of ERC-1155.
